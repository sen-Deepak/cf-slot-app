/**
 * todays-shoots.js - Today's Shoots page logic
 */
...
async function loadAttendanceSection() {
    ...
    try {
        ...
        // Get today's date in format "02 Mar 26"
        const today = formatDateForApi(new Date());
        
        // Fetch attendance data for a wider range to get today's data
        const dateTo = new Date();
        dateTo.setDate(dateTo.getDate() + 1);
        const dateToStr = formatDateForApi(dateTo);
        
        // Build URL to Google Apps Script directly
        const apiUrl = `${gasUrl}?action=read_all&from=${encodeURIComponent(today)}&to=${encodeURIComponent(dateToStr)}`;
        ...
        const todayRecords = data.rows.filter(row => row.Date === today);
        
        if (todayRecords.length === 0) {
            attendanceContent.innerHTML = '<p style="color: #999; text-align: center;">No attendance records for today</p>';
            return;
        }
...
/**
 * Helper: Format date for API (e.g., "02 Mar 26")
 */
...
/**
 * Load Today's Shoots Schedule - Timeline with API data
 */
